...
collect_header [] targ True = " |- " ++ show (Neg (Neg targ))
collect_header [] targ False = "|- " ++ show (Neg (Neg targ))

glivenko_print :: [Expr] -> IO()
glivenko_print (hd : tl) = do
  putStrLn (show hd)
  glivenko_print tl
glivenko_print [] = do
  return ()
...
success new_st hypotheses_raw targetExpression= do
  ...
  if needHeader proof 
    then putStrLn (collect_header hypotheses_raw targetExpression False)
    else return ()
  let changed = glivenko_change proof
  glivenko_print changed
